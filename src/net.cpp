...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Antoinettecoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"antoinettecoin.io", "37.139.26.19"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Antoinettecoin is probably already running."), addrBind.ToString().c_str());
        else
